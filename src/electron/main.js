...
async function ejecutarConConsolaVisible(fn) {
    ...
    console.log = (...args) => { logOriginal(...args); emitirProgreso(...args); };
    console.error = (...args) => { errorOriginal(...args); emitirProgreso("ERROR:", ...args); };
    console.warn = (...args) => { warnOriginal(...args); emitirProgreso("ADVERTENCIA:", ...args); };
    try { return await fn(); }
    finally { console.log = logOriginal; console.error = errorOriginal; console.warn = warnOriginal; }
}
...
app.whenReady().then(() => {
    ...
    app.on("activate", () => { if (BrowserWindow.getAllWindows().length === 0) crearVentanaPrincipal(); });
});
...
ipcMain.handle("seleccionarCSV", async (event, carpeta) => {
    ...
    if (path.dirname(rutaCSV) !== rutaCarpeta) throw new Error("El CSV seleccionado debe estar dentro de la carpeta del proyecto.");
    return rutaCSV;
});
...
function serializarProyecto(proyecto) {
    ...
    return {
        ...
        listaFotografias: proyecto.obtenerFotografias().map(foto => ({ nombre: foto.nombre, ruta: foto.ruta, extension: foto.extension, tamano: foto.tamano, enGaleria: foto.enGaleria, esHero: foto.esHero }))
    };
}
...
ipcMain.handle("ejecutarProyecto", async (event, carpeta, rutaCSV) => {
    if (!carpeta) throw new Error("No se recibió una carpeta de proyecto.");
    if (!rutaCSV) throw new Error("No se recibió el CSV fuente del proyecto.");
    ...
    return ejecutarConConsolaVisible(async () => {
        ...
        if (proyecto.flujoEditorial === "EDITORIAL_PORTFOLIO") {
            ...
            const resultado = await directorPortfolio.ejecutar(proyecto);
...
ipcMain.handle("mostrarSalidaEditorial", async (event, rutaSalida) => {
    if (!rutaSalida) throw new Error("No existe una ruta de salida editorial para mostrar.");
