...
async function ejecutar() {
    ...
    const idsFotografias = ["foto-01.jpg", "foto-02.jpg"];

    const resultado = await comprensor.comprender(contexto, idsFotografias);
    ...
    const respuestaConFotoInexistente = {
        async generarTexto() {
            return JSON.stringify({
                ...JSON.parse(respuestaValida),
                materialidad: [
                    {
                        texto: "Afirmación con fotografía inexistente.",
                        evidencia: ["foto-999.jpg"]
                    }
                ]
            });
        }
    ...
    const comprensorFotoInexistente = new ComprensorEditorialPortfolio({
        clienteIA: respuestaConFotoInexistente
    });

    await assert.rejects(
        () => comprensorFotoInexistente.comprender(contexto, idsFotografias),
        /La fotografía de evidencia "foto-999\.jpg" no existe en la evidencia visual disponible\./
    );
