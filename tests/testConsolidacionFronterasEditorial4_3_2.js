...
assert.ok(processor.includes("No existe evidencia visual válida"));
...
assert.ok(processor.includes("const evidenciaFoto = proyecto.expediente.observacionesVision.find"));
assert.ok(processor.includes("generar(`foto_${i + 1}_editorial`"));
console.log("✓ La galería se recorre fotografía por fotografía, cada evidencia se recupera por nombre y cada fotografía recibe una única etapa editorial.\n");
